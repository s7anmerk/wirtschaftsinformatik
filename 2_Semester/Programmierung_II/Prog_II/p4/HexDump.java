...
public class HexDump
{
  ...
  /* produces a String in the Form of a hexdumpls
  ...
  public String getHexString()
  {
    ...
    for(int i =0;i<data.length;i+=16)
    {
      ...
      {
        s+=new String(hexByte(data[i+j],2))+' ';    //+new String(hexByte(data[i+j],4))+" | ";
        //s+=new String(hexByte(data[i+j],4))+" | ";
      }
      for(int j =0; j<16 && (i+j) <  data.length; j++)
      {
        s+=(char)data[i+j];         //+ new String(hexByte(data[j],2))+' ';
      }
    }
    ...
    //  s+= hexByte(data[i], 2) + " " + hexByte(data[i], 2) + " " + hexByte(data[i], 2) + " " + hexByte(data[i], 2) + " " + "\n";
    
    /*
    s+= hexByte(data[i], 2) + " " + hexByte(data[i], 2) + " " + hexByte(data[i], 2) + " " + hexByte(data[i], 2) + " | " + hexByte(data[i], 2) + " " + hexByte(data[i], $
    */
