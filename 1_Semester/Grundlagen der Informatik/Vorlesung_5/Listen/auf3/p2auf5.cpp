...
class Name {
	class el { // Listenelement
		public:
			   ...
			   el(char *p=0):p(p?strcpy(new char[strlen(p)+1],p):0),pe(0){
				   cout<<"Konstruktor el: "<<this->p?this->p:""<<'\n'; 
			   }

			   ~el(){	cout<<"Destruktor el: "<<this->p?this->p:""<<'\n';
						delete [] p; p=0; pe=0; 
			   }
	};
...
public:
	...
	*/
	...
	void addfirst(char *z){	// neuer Namen an den Listenanfang anfuegen (guenstig)
		el *pold = pel;
		pel = new el(z); 
		pel->pe = pold;
	}

	void addlast(char *z){ // neuer Namen an das Listenende anfuegen (unguenstig)
		el *pp=pel;
		if(!pel) pel=new el(z);
		else {
